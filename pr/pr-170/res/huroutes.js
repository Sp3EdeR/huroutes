...
overlay.remove();});});map.on('overlayadd',(overlay)=>{var overlays=localStorage.overlays;overlays=overlays?overlays.split('|'):[];overlays.push(overlay.layer.id);localStorage.overlays=overlays.join('|');});map.on('overlayremove',(overlay)=>{var overlays=(localStorage.overlays||'').split('|');const idx=overlays.indexOf(overlay.layer.id);if(idx!=-1)
...
var navigation={provs:huroutes.opt.navLinkProviders,getId:function(){return this.provs[localStorage.navprovider]?localStorage.navprovider:Object.keys(this.provs)[0];},getLink:function(coords,reverse){const dst=reverse?coords[0]:coords[coords.length-1];const wpts=this._getWaypoints(coords,reverse);return this._getProv().getLink(dst,wpts);},_getProv:function(){return this.provs[this.getId()];},_getWaypoints:function(coords,reverse){const count=this._getProv().waypoints;if(!count)
return[];let idx=reverse?coords.length-1:0;const step=Math.floor(coords.length/count)*(reverse?-1:1);let wpts=[];for(i=0;i<count;++i,idx+=step)
wpts.push(coords[idx]);return wpts;},}
...
localStorage.overlays=convertEach(langDict.map,localStorage.overlays);if(localStorage.theme&&!(localStorage.theme in huroutes.opt.themes))
