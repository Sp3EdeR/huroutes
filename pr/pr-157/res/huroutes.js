...
{provider=huroutes.opt.l10n.providers.Google;sourceLang=$('html').attr('lang');lang=provider.getCurrentLang()||sourceLang;langIdx=provider.langs.findIndex(lng=>lng[0]==lang);lang=0<=langIdx?provider.langs.splice(langIdx,1):provider.langs[0];$('#sidebar .dropdown-toggle').append($('<span class="fi fi-{0}">'.format(lang[0])));$('#sidebar .dropdown-menu').append(provider.langs.map(lang=>{var url=($('head base[href]').attr('href')||location.origin+location.pathname);if(lang[0]!=sourceLang)
