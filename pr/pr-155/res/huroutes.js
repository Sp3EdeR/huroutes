...
tileOverlay.addTo(map);map.createPane('bkgRoutes');map.getPane('bkgRoutes').style.zIndex=450;gtPathPrefix=0<$('head base[href]').length?'huroutes/':'';$.getJSON(gtPathPrefix+'data.json',initializeContent).fail(function(){console.error('Failed loading the route database.');});initColorSelector();initCtrls(tiles,overlays);$('#options-dialog').on('hidden.bs.modal',updateOptions);$('.options-button [title]').initTooltip();initSidebarEvents();initNavSelector();initDownloadTypeSelector();initAdToast();navigateTo(fragment.get())});var stopFollowingLocation=()=>{};function initCtrls(tiles,overlays)
