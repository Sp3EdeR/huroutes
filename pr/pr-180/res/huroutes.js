...
{window.addEventListener("beforeinstallprompt",event=>setTimeout(()=>pwaToast(event),5000));if(navigator.userAgent&&navigator.userAgent.match(/(?:Macintosh|iP(?:od|ad|hone))(?!.*(?:Chrome|Firefox|CriOS|FxiOS))/)&&!navigator.standalone&&!window.matchMedia('(display-mode: standalone)').matches)
