...
{const shareRoute=routeId=>{navigator.share({title:routeId.slice(1),url:location.href.split("#")[0]+routeId});}
var eDownload=$('\
<div class="btn-group mr-2" role="group">\
...
</div>'.format(langDict.dlRouteTooltip,langDict.shareTooltip,routeId));eDownload.find('.download').click(()=>dlRoute.download(coords,routeId)??false).tooltip();eDownload.find('.share').click(e=>{shareRoute($(e.currentTarget).attr('href'));return false;}).tooltip();elem.append(eDownload);}
